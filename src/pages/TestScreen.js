...
function TestScreen() {
  ...
  async function fetchExamAndQuestions() {
    try {
      ...
      const { data: questionsData, error: questionsError } = await supabase
        .from('questions').select('*').eq('exam_id', examId).order('created_at')
      if (questionsError) throw questionsError
      if (questionsData.length === 0) throw new Error('No questions found for this exam.')
  ...
  async function saveResults(completedAnswers) {
    try {
      ...
      const scorePercent = Math.round((correctCount / completedAnswers.length) * 100)
    ...
    <div style={styles.page}>
      ...
      <div style={styles.container}>
        <div style={styles.card}>
          ...
          {hasSubmitted && (
            <div style={isCorrect ? styles.explanationCorrect : styles.explanationWrong}>
              <p style={styles.resultLabel}>
                {isCorrect ? '✅ Correct!' : `❌ Correct answer: ${currentQuestion.correct_option.toUpperCase()}`}
              </p>
          ...
          {!hasSubmitted ? (
            <button
              style={{ ...styles.actionButton, opacity: selectedOption ? 1 : 0.35 }}
            ...
            <button style={styles.actionButton} onClick={handleNext}>
              {currentIndex === questions.length - 1 ? 'See My Results →' : 'Next Question →'}
            </button>
          )}
