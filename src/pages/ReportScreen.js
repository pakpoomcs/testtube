...
function ReportScreen() {
  ...
  const { answers, exam } = location.state
  ...
  function getGrade() {
    if (scorePercent >= 90) return { grade: 'A', color: colors.success, message: 'Outstanding! You have a strong command of this material.' }
    if (scorePercent >= 75) return { grade: 'B', color: colors.teal, message: 'Great work! A little more practice on your weak topics and you\'ll ace it.' }
    if (scorePercent >= 60) return { grade: 'C', color: colors.warning, message: 'Decent effort. Focus on the topics highlighted below to push your score up.' }
    return { grade: 'D', color: colors.danger, message: 'Keep going — review each explanation carefully and try again. Progress takes practice.' }
  }
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.topBar}>
        <button style={styles.backButton} onClick={() => navigate('/')}>← Exams</button>
      ...
      <div style={styles.content}>
              ...
              return (
                <div key={topic} style={styles.topicRow}>
                  <div style={styles.topicMeta}>
                    ...
                    <span style={{ ...styles.topicPct, color: barColor }}>{pct}%</span>
                  </div>
        ...
        <div style={styles.section}>
          ...
          {Object.entries(topicBreakdown).filter(([, s]) => s.correct / s.total < 0.75).length === 0 ? (
            <p style={styles.suggestionText}>No major weak areas — try a harder difficulty next time!</p>
          ) : (
        ...
        <div style={styles.section}>
          ...
          <div style={styles.reviewList}>
            {answers.map((answer, i) => (
              <div key={i} style={{
                ...styles.reviewCard,
                borderLeft: `4px solid ${answer.isCorrect ? colors.success : colors.danger}`
              }}>
                <p style={styles.reviewQ}>{i + 1}. {answer.question.question_text}</p>
                <p style={{ ...styles.reviewResult, color: answer.isCorrect ? colors.success : colors.danger }}>
                  {answer.isCorrect ? '✅ Correct' : `❌ You chose ${answer.selectedOption.toUpperCase()} · Correct: ${answer.question.correct_option.toUpperCase()}`}
                </p>
                {!answer.isCorrect && (
                  <p style={styles.reviewExplanation}>{answer.question.explanation}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div style={styles.buttonRow}>
          ...
          <button style={styles.buttonPrimary} onClick={() => navigate(`/test/${exam.id}`)}>
            Try Again
          </button>
        </div>
